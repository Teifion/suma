defmodule Fusion.RAG.ComparisonLib do
  @moduledoc """

  """

  # alias Fusion.RAG.Comparison
  ...
  def comparison_topic(comparison_id) do
    "Fusion.Comparison:#{comparison_id}"
  end

  @spec start_comparison_server(map()) :: {:ok, pid()} | {:error, String.t()}
  def start_comparison_server(params) do
    {:ok, pid} = DynamicSupervisor.start_child(Fusion.ComparisonSupervisor, {
      Fusion.RAG.ComparisonServer,
      name: "comparison_#{Fusion.uuid()}",
      data: %{
        params: params,
        id_callback: self()
      }
  ...
  def list_local_comparison_ids() do
    Registry.select(Fusion.LocalComparisonRegistry, [{{:"$1", :_, :_}, [], [:"$1"]}])
  end
  ...
  """
  @spec add_player_to_comparison(Fusion.user_id(), Comparison.id()) :: :ok | {:error, String.t()}
  ...
  Adds a player to the comparison
  """
  @spec remove_player_from_comparison(Fusion.user_id(), Comparison.id()) :: :ok | nil
  ...
  def comparison_exists?(comparison_id) do
    case Registry.lookup(Fusion.LocalComparisonRegistry, comparison_id) do
      [{_pid, _}] -> true
      _ -> false
    end
  end
  ...
  def stop_comparison_server(comparison_id) do
    case get_comparison_pid(comparison_id) do
      ...
      p ->
        # Fusion.broadcast(comparison_topic(comparison_id), %{
        #   event: :comparison_closed,
        #   comparison_id: comparison_id
        # })

        DynamicSupervisor.terminate_child(Fusion.ComparisonSupervisor, p)
        :ok
    end
  end
end

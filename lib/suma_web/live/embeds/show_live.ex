defmodule SumaWeb.Embeds.ShowLive do
  @moduledoc false
  ...
  @impl true
  def handle_info(
        %{topic: "Suma.Embeds.User:" <> _, event: :client_updated} = msg,
        socket
      ) do
  ...
  def handle_info(%{topic: "Suma.Embeds.User:" <> _}, socket) do
    socket
    |> noreply()
  end

  def handle_info(
        {SumaWeb.Embeds.UserFormComponent, {:updated_changeset, %{changes: _changes}}},
        socket
      ) do
    {:noreply, socket}
  end
  ...
  def handle_event("delete-user-token", %{"token_id" => token_id}, socket) do
    token = Suma.Embeds.get_user_token(token_id)
    Suma.Embeds.delete_user_token(token)
  ...
  defp get_other_data(%{assigns: %{user: user}} = socket) do
    ...
    |> assign(
      :tokens,
      Suma.Embeds.list_user_tokens(where: [user_id: user.id], order_by: ["Most recently used"])
    )
  end
end

defmodule SumaWeb.CompareLive.Show do
  use SumaWeb, :live_view

  alias Suma.RAG.{Completion, ComparisonLib, ModelLib}
  ...
  def mount(%{"id" => id}, _session, socket) when is_connected?(socket) do
    case ComparisonLib.get_comparison(id) do
      ...
      comparison_state ->
        :ok = Suma.subscribe(ComparisonLib.comparison_topic(id))
  ...
  def handle_info(%{topic: "Suma.Comparison:" <> _, event: :new_key_difference} = msg, socket) do
    socket
  ...
  def handle_info(%{topic: "Suma.Comparison:" <> _, event: :new_variable} = msg, socket) do
    socket
    |> assign(:variables, msg.variables)
    |> assign(:adding_variable?, false)
    |> noreply
  ...
  def handle_info(%{topic: "Suma.Comparison:" <> _, event: :removed_variable} = msg, socket) do
    socket
    |> assign(:variables, msg.variables)
    |> noreply
  end

  def handle_info(%{topic: "Suma.Comparison:" <> _, event: :awaiting_completions} = msg, socket) do
    socket
    |> assign(:responses, %{})
    |> noreply
  end

  def handle_info(%{topic: "Suma.Comparison:" <> _, event: :new_response} = msg, socket) do
    socket
    |> assign(:responses, Map.put(socket.assigns.responses, msg.variable_value, msg.response))
    |> noreply
  end

  def handle_info(%{topic: "Suma.Comparison:" <> _} = msg, socket) do

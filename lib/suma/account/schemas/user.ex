defmodule Suma.Account.User do
  @moduledoc """
  # User
  The database representation of a person using/playing your game; not to be confused with `Suma.Connections.Client` which represents their logged in and online state.

  ### Attributes

  * `:name` - The name of the user
  * `:email` - The email of the user
  * `:password` - The encrypted password of the user; if you ever want to compare the password please make user of `valid_password?/2` (which can be called from `Suma.Account`).
  ...
  * `:social_score` - Numerical score representing how much the user is liked or disliked by other players in general
  ...
  schema "account_users" do
    ...
    has_many :tokens, Suma.Account.UserToken
  ...
  defp validate_password(changeset) do
    min_length = Application.get_env(:suma, :default_min_user_password_length, 6)
  ...
  def calculate_user_permissions(changeset) do
    if Application.get_env(:suma, :fn_calculate_user_permissions) do
      f = Application.get_env(:suma, :fn_calculate_user_permissions)
      f.(changeset)
    else

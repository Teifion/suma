defmodule Suma.Account.UserLib do
  @moduledoc """
  Library of user related functions.
  """
  use SumaMacros, :library
  alias Suma.Account.{User, UserQueries}

  @doc false
  @spec user_topic(Suma.user_id() | User.t()) :: String.t()
  def user_topic(%User{id: user_id}), do: "Suma.Account.User:#{user_id}"
  def user_topic(user_id), do: "Suma.Account.User:#{user_id}"
  ...
  """
  @spec list_users(Suma.query_args()) :: [User.t()]
  ...
  ## Examples
      ...
      nil
  """
  @spec get_user_by_id(Suma.user_id()) :: User.t() | nil
  ...
  @spec do_get_user_by_id(Suma.user_id()) :: User.t() | nil
  ...
  ## Examples
      ...
      iex> get_user_by_email("noodle@Suma")
      %User{}
  ...
  defp maybe_decache_user({:ok, user}) do
    Suma.invalidate_cache(:user_by_user_id_cache, user.id)
    {:ok, user}
  end
  ...
  def register_failed_login(user_id, ip, reason) do
    ...
    :telemetry.execute(
      [:suma, :user, :failed_login],
      %{reason: reason},
      %{user_id: user_id, ip: ip}
    )

    # TODO: Call out to the audit log system
    # Suma.create_audit_log(user_id, ip, "failed-login", %{reason: reason})
  ...
  defp allow_ip_login_attempt?(ip) do
    ...
    # max_allowed_ip = Suma.get_server_setting_value("login.ip_rate_limit")
  ...
  defp allow_user_login_attempt?(userid) do
    ...
    # max_allowed_user = Suma.Settings.get_server_setting_value("login.user_rate_limit")
  ...
  ## Examples
    ...
    false
  """
  @spec allow?(Suma.user_id() | User.t(), [String.t()] | String.t()) :: boolean

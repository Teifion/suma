defmodule FusionWeb.RAG.EmbedFormComponent do
  @moduledoc false
  ...
  def update(%{user: user} = assigns, socket) do
    changeset = RAG.change_user(user)
  ...
  defp save_user(socket, :edit, user_params) do
    case RAG.update_user(socket.assigns.user, user_params) do
      {:ok, user} ->
        notify_parent({:saved, user})

        {:noreply,
         socket
         |> put_flash(:info, "Embed updated successfully")
  ...
  defp save_user(socket, :new, user_params) do
    ...
    case RAG.create_user(user_params) do
      {:ok, user} ->
        notify_parent({:saved, user})

defmodule FusionWeb.Embeds.ShowLive do
  @moduledoc false
  ...
  @impl true
  def handle_info(
        %{topic: "Fusion.Embeds.User:" <> _, event: :client_updated} = msg,
        socket
      ) do
  ...
  def handle_info(%{topic: "Fusion.Embeds.User:" <> _}, socket) do
    socket
    |> noreply()
  end

  def handle_info(
        {FusionWeb.Embeds.UserFormComponent, {:updated_changeset, %{changes: _changes}}},
        socket
      ) do
    {:noreply, socket}
  end
  ...
  def handle_event("delete-user-token", %{"token_id" => token_id}, socket) do
    token = Fusion.Embeds.get_user_token(token_id)
    Fusion.Embeds.delete_user_token(token)
  ...
  defp get_user(%{assigns: %{user_id: user_id}} = socket) do
    user =
      try do
        Fusion.Embeds.get_user(user_id)
      rescue
  ...
  defp get_other_data(%{assigns: %{user: user}} = socket) do
    ...
    |> assign(
      :tokens,
      Fusion.Embeds.list_user_tokens(where: [user_id: user.id], order_by: ["Most recently used"])
    )
  end
end

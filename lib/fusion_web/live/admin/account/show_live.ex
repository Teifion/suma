defmodule FusionWeb.Admin.Account.ShowLive do
  @moduledoc false
  ...
  def mount(%{"user_id" => user_id}, _session, socket) when is_connected?(socket) do
    socket =
      ...
      # |> assign(:client, Fusion.Account.get_client(user_id))
      |> get_user
      |> get_other_data
  ...
  @impl true
  def handle_info(
        %{topic: "Fusion.Account.User:" <> _, event: :client_updated} = msg,
        socket
      ) do
  ...
  def handle_info(%{topic: "Fusion.Account.User:" <> _}, socket) do
    socket
    |> noreply()
  end

  def handle_info(
        {FusionWeb.Account.UserFormComponent, {:updated_changeset, %{changes: _changes}}},
        socket
      ) do
    {:noreply, socket}
  end
  ...
  def handle_event("delete-user-token", %{"token_id" => token_id}, socket) do
    token = Fusion.Account.get_user_token(token_id)
    Fusion.Account.delete_user_token(token)
  ...
  defp get_user(%{assigns: %{user_id: user_id}} = socket) do
    user =
      try do
        Fusion.Account.get_user(user_id)
      rescue
  ...
  defp get_other_data(%{assigns: %{user: user}} = socket) do
    ...
    |> assign(
      :tokens,
      Fusion.Account.list_user_tokens(where: [user_id: user.id], order_by: ["Most recently used"])
    )
  end
end

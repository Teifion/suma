defmodule FusionWeb.UserAuth do
  use FusionWeb, :verified_routes

  import Plug.Conn
  import Phoenix.Controller

  alias Fusion.Account

  # Make the remember me cookie valid for 60 days.
  # If you want bump or reduce this value, also change
  # the token expiry itself in UserToken.
  @max_age 60 * 60 * 24 * 60
  @remember_me_cookie "_Fusion_user_remember_me"
  ...
  def log_in_with_token(conn, %Fusion.Account.UserToken{} = token, params \\ %{}) do
    user_return_to = get_session(conn, :user_return_to)
  ...
  def log_out_user(conn) do
    ...
    cached_user_token &&
      Fusion.invalidate_cache(:user_token_identifier_cache, cached_user_token.identifier_code)

    if live_socket_id = get_session(conn, :live_socket_id) do
      FusionWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  the current_user:
      ...
      defmodule FusionWeb.PageLive do
        use FusionWeb, :live_view

        on_mount {FusionWeb.UserAuth, :mount_current_user}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{FusionWeb.UserAuth, :ensure_authenticated}] do
